...
void applyBoundaryConditions(double* rho, double* u, double* p, 
    ...
    if (left_bc == "wall") {
        ...
        for (int i = 0; i < ghost_cells; i++) {
            ...
            u[i] = -u[ghost_cells];  // отражение скорости
            p[i] = p[ghost_cells];
        }
    }
    else if (left_bc == "flow") {
        // Поток - постоянные значения
        for (int i = 0; i < ghost_cells; i++) {
            rho[i] = rho[ghost_cells];
            u[i] = u[ghost_cells];
            p[i] = p[ghost_cells];
        ...
        // Периодические - копируем с правого края
        for (int i = 0; i < ghost_cells; i++) {
    ...
    if (right_bc == "wall") {
        ...
        // Периодические - копируем с левого края
        for (int i = 0; i < ghost_cells; i++) {
    ...
    std::cout << "Boundary conditions: left=" << left_bc << ", right=" << right_bc << std::endl;
}