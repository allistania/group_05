...
// ------- Геометрическое определение типа границы ----------
int getBcTypeFromPosition(double mx, double my) {
    const double eps = 0.1;
    // Профиль: X около [0,1] и Y мало
    ...
    if (fabs(mx - 8.0) < eps)
        return 3; // outflow
    // Верх и низ – стенка
    if (fabs(my - 2.0) < eps || fabs(my - (-2.0)) < eps)
        return 1; // wall
    // На всякий случай – стенка
    return 1;
}
...
void computeInletFlux(const array<double,4>& UL, double nx, double ny,
    ...
    double uR   = INLET_U;
    double vR   = INLET_V;
    double pR   = pL;   // экстраполяция давления
...
void computeCpDistribution(const vector<Point>& points,
                           ...
                           const string& outputDir,
                           const string& baseName) {
    // Файл для сырых данных: x, y, x/c, Cp, сторона (U=верх, L=низ)
    string filename = outputDir + "/" + baseName + "_cp_distribution.dat";
    ofstream out(filename);
    if (!out) {
        cerr << "Cannot create Cp distribution file.\n";
        return;
    }
    out << "# x y x_over_c Cp side\n";
    out << "# side: U = upper surface, L = lower surface\n";

    double q_inf = 0.5 * rho_inf * u_inf * u_inf;

    for (const auto& edge : edges) {
        // Только стенка (bcTag == 1)
        ...
        // Координаты середины грани
        double mx = 0.5 * (points[i1].x + points[i2].x);
        double my = 0.5 * (points[i1].y + points[i2].y);
        // Давление в примыкающей ячейке
        double rho, u, v, p;
        conservativeToPrimitive(U[edge.cellA], rho, u, v, p);
        double Cp = (p - p_inf) / q_inf;
        double x_over_c = mx / chord;   // chord = 1.0 по условию, но можно вычислить реальную

        // Определяем сторону: верхняя (U) если my > 0, иначе нижняя (L)
        // Для профиля NACA с малым углом атаки это корректно.
    ...
    out.close();
    cout << "Cp distribution saved to: " << filename << "\n";

    // Дополнительно создаём два файла: верх и низ (для удобного построения)
    string upperFile = outputDir + "/" + baseName + "_cp_upper.dat";
    string lowerFile = outputDir + "/" + baseName + "_cp_lower.dat";
    ofstream up(upperFile), low(lowerFile);
    if (!up || !low) {
        cerr << "Could not create separate Cp files.\n";
        return;
    }
    up << "# x_over_c Cp\n";
    low << "# x_over_c Cp\n";

    // Повторно пройдём по граням и разнесём
    for (const auto& edge : edges) {
        if (edge.bcTag != 1) continue;
        const Triangle& cell = triangles[edge.cellA];
        int i1 = cell.nodes[edge.edgeIdxA];
    ...
    up.close();
    low.close();
    cout << "Separated Cp data written: " << upperFile << " and " << lowerFile << "\n";
}
