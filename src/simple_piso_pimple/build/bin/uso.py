...
def create_all_vars_gif(frame_dir, output_dir, duration=100, slice_y=None, num_levels=50):
    ...
    for i, f in enumerate(files):
        ...
        for idx, var in enumerate(variables):
            ...
            if slice_y is None:
                ...
                # явно задаЄм уровни от минимума до максимума
                levels = np.linspace(z.min(), z.max(), num_levels)
                if np.allclose(z, z[0]):
                    ax.text(0.5, 0.5, f'{z[0]:.2e} (constant)', 
...
def main():
    parser = argparse.ArgumentParser(description="Create a GIF with all variables from simulation frames (using contour lines)")
    ...
    parser.add_argument('--num_levels', type=int, default=50, help='Number of contour levels (default: 50)')
    args = parser.parse_args()

    create_all_vars_gif(args.frame_dir, args.output_dir, args.duration, args.slice_y, args.num_levels)
