...
# -------------------------------------------------------------------
# 6. EXTRA: Save only the velocity magnitude as a separate file
# -------------------------------------------------------------------
fig_speed = plt.figure(figsize=(20, 4))          
ax_speed = fig_speed.add_subplot(111)
im_speed = ax_speed.pcolormesh(X, Y, speed, shading='auto', cmap='viridis')
ax_speed.set_title(r'Velocity magnitude $\sqrt{u^2+v^2}$', fontsize=14)
ax_speed.set_xlabel('x')
ax_speed.set_ylabel('y')
ax_speed.set_aspect('equal')                    
plt.colorbar(im_speed, ax=ax_speed)
plt.tight_layout()
plt.savefig('plots/godunov2D_speed_magnitude.png', dpi=150)
plt.close(fig_speed)