...
// ========== ГЛОБАЛЬНЫЕ ПАРАМЕТРЫ (изменяемые) ==========
double GAMMA = 1.4;
double CFL = 0.8;
...
double INLET_V = 0.0;
double INLET_E = 0.0;
string meshFileName = "corrected.msh";

const double CHORD = 1.0;   // неизменяемая константа// ------- Геометрическое определение типа границы ----------
int getBcTypeFromPosition(double mx, double my) {
    const double eps = 0.1;
    // Профиль (стенка)
    if (mx > -0.2 && mx < 1.2 && fabs(my) < 0.3)
        return 1; // wall
    ...
    // Верхняя и нижняя границы – делаем их открытыми (выход)
    // Поток горизонтальный, поэтому на верхней и нижней границах лучше задать outflow.
    if (fabs(my - 8.0) < eps || fabs(my + 8.0) < eps)
        return 3; // outflow
...
void readParameters(const string& filename) {
    ...
    if (!file) {
        cerr << "Warning: cannot open parameter file '" << filename << "'. Using default values.\n";
        return;
    }
    string line;
    while (getline(file, line)) {
        ...
        // Преобразуем значение в double (или строку для mesh_file)
        if (key == "mesh_file") {
            meshFileName = value;
        } else if (key == "mach_inf") {
            MACH_INF = stod(value);
        } else if (key == "alpha_deg") {
            ALPHA_DEG = stod(value);
        } else if (key == "final_time") {
            FINAL_TIME = stod(value);
        } else if (key == "output_interval") {
            OUTPUT_INTERVAL = stod(value);
        } else if (key == "cfl") {
            CFL = stod(value);
        } else if (key == "gamma") {
            GAMMA = stod(value);
        } else if (key == "inlet_rho") {
            INLET_RHO = stod(value);
        } else if (key == "inlet_p") {
            INLET_P = stod(value);
        } else if (key == "outlet_p") {
            OUTLET_P = stod(value);
        } else {
            cerr << "Unknown parameter key: " << key << endl;
        }
    }
    file.close();
}
...
void buildEdges(const vector<Point>& points,
    ...
    // Теперь назначаем bcTag по координатам для граничных рёбер (cellB == -1)
for (auto& edge : edges) {
    ...
    edge.bcTag = getBcTypeFromPosition(mx, my);
    if (edge.cellB != -1) {
        // Внутреннее ребро: нормаль должна быть направлена из cellA в cellB
        double cxA = tA.centroid[0], cyA = tA.centroid[1];
        double cxB = triangles[edge.cellB].centroid[0], cyB = triangles[edge.cellB].centroid[1];
        double dx_c = cxB - cxA, dy_c = cyB - cyA;
        double dot = nx * dx_c + ny * dy_c;
        if (dot < 0) { nx = -nx; ny = -ny; }
    } else {
        // Граничное ребро: нормаль должна указывать наружу (от ячейки)
        double cx = tA.centroid[0], cy = tA.centroid[1];
        double dx_c = cx - mx, dy_c = cy - my;
        double dot = nx * dx_c + ny * dy_c;
        if (dot > 0) { nx = -nx; ny = -ny; }   // наружу
    }
    edge.nx = nx;
    edge.ny = ny;
}
...
void computeSymmetryFlux(const array<double,4>& UL, double nx, double ny, array<double,4>& flux) {
    // Отражение: нормальная скорость меняет знак, касательная сохраняется
    double rho, u, v, p;
    conservativeToPrimitive(UL, rho, u, v, p);
    double uNorm = u*nx + v*ny;
    double uTang = -u*ny + v*nx;
    double uNormWall = -uNorm;   // отражение
    double uFace = uNormWall * nx - uTang * ny;
    double vFace = uNormWall * ny + uTang * nx;
    flux[0] = 0.0;
    flux[1] = p * nx;
    flux[2] = p * ny;
    flux[3] = 0.0;   // для симметрии – то же, что и стенка, так как нормальный поток массы и энергии = 0
}
...
void computeInletFlux(const array<double,4>& UL, double nx, double ny,
    ...
    // Внешнее состояние: задаём скорость и плотность набегающего потока,
    // давление экстраполируем изнутри, чтобы избежать отражений.
    double rhoR = INLET_RHO;
    double uR   = INLET_U;
    double vR   = INLET_V;
    double pR   = INLET_P;   // экстраполяция давления
    //double pR   = pL;
    // Нормальные и тангенциальные компоненты
    double uNormL = uL * nx + vL * ny;
    // Внешняя нормальная скорость должна быть направлена внутрь, т.е. против нормали
    double uNormR = (uR * nx + vR * ny);
    double uTangL = -uL * ny + vL * nx;
    double uTangR = -uR * ny + vR * nx;
    ...
    // Тангенциальная скорость: берём из набегающего потока (как обычно для входа)
...
double computeTimeStep(const vector<Triangle>& triangles,
    ...
    for (size_t i = 0; i < triangles.size(); ++i) {
        ...
        for (int e = 0; e < 3; ++e) {
            int edgeIdx = triangles[i].edges[e];
            const Edge& edge = edges[edgeIdx];
            // Скорость + скорость звука в проекции на нормаль
            double u_n = fabs(u * edge.nx + v * edge.ny);
            double spectral_radius = u_n + c;
            lambda_max += spectral_radius * edge.length;
        }
        double dt_cell = 2.0 * triangles[i].area / lambda_max;
...
void updateSolution(vector<array<double,4>>& U,
                    const vector<Triangle>& triangles,
                    const vector<Edge>& edges,
                    const vector<Point>& points,   // <-- нужно передать points
                    double dt) {
    ...
    for (const auto& edge : edges) {
        ...
        if (isBoundary) {
            // Определяем тип границы по координатам середины ребра
            const Triangle& cell = triangles[edge.cellA];
            int i1 = cell.nodes[edge.edgeIdxA];
            int i2 = cell.nodes[(edge.edgeIdxA + 1) % 3];
            double mx = 0.5 * (points[i1].x + points[i2].x);
            double my = 0.5 * (points[i1].y + points[i2].y);
            int bc = getBcTypeFromPosition(mx, my);
            ...
            if (bc == 1) {
                ...
                computeWallFlux(UL, edge.nx, edge.ny, flux); // fallback
            }
        } else {
...
void computeCpDistribution(const vector<Point>& points,
                           const vector<Triangle>& triangles,
                           const vector<Edge>& edges,
                           const vector<array<double,4>>& U,
                           ...
                           const string& outputDir,
                           const string& baseName) {
    string filename = outputDir + "/" + baseName + "_cp_distribution.dat";
    ofstream out(filename);
    out << "# x y x_over_c Cp side\n";

    double q_inf = 0.5 * rho_inf * u_inf * u_inf;

    for (const auto& edge : edges) {
        if (edge.cellB != -1) continue; // только граничные рёбра
        ...
        // Фильтр: только поверхность профиля (x от -0.1 до 1.1, |y| < 0.2)
        if (mx < -0.1 || mx > 1.1 || fabs(my) > 0.2) continue;

        double rho, u, v, p;
        conservativeToPrimitive(U[edge.cellA], rho, u, v, p);
        double Cp = (p - p_inf) / q_inf;
        double x_over_c = mx / chord;
        char side = (my > 0.0) ? 'U' : 'L';
        out << mx << " " << my << " " << x_over_c << " " << Cp << " " << side << "\n";
    }
    out.close();
    cout << "Cp distribution saved to: " << filename << " (profiles found: check file)\n";
}
...
int main(int argc, char* argv[]) {
    // Если в командной строке передан файл параметров, использовать его
    string paramFile = "run.txt";
    if (argc > 1) paramFile = argv[1];
    
    readParameters(paramFile);
        // Отладочный вывод (можно убрать позже)
    cout << "=== START PARAMETRES ===\n";
    cout << "mesh_file = " << meshFileName << "\n";
    cout << "MACH_INF = " << MACH_INF << "\n";
    cout << "ALPHA_DEG = " << ALPHA_DEG << "\n";
    cout << "FINAL_TIME = " << FINAL_TIME << "\n";
    cout << "OUTPUT_INTERVAL = " << OUTPUT_INTERVAL << "\n";
    cout << "CFL = " << CFL << "\n";
    cout << "GAMMA = " << GAMMA << "\n";
    cout << "INLET_RHO = " << INLET_RHO << "\n";
    cout << "INLET_P = " << INLET_P << "\n";
    cout << "OUTLET_P = " << OUTLET_P << "\n";
    // Вычисление производных параметров
    SOUND_SPEED = sqrt(GAMMA * INLET_P / INLET_RHO);
    INLET_U = MACH_INF * SOUND_SPEED;
    INLET_V = 0.0;
    INLET_E = INLET_P / (GAMMA - 1.0) + 0.5 * INLET_RHO * (INLET_U * INLET_U);
    
    // Далее используем meshFileName для загрузки сетки
    vector<Point> points;
    vector<Triangle> triangles;
    vector<BoundaryLine> boundaryLines;
    if (!readGmshMesh(meshFileName, points, triangles, boundaryLines)) return 1;
    ...
    while (t < FINAL_TIME) {
        ...
        updateSolution(U, triangles, edges, points, dt);
        t += dt;
        iter++;
        if (t >= nextOutput) {
...
// Вычисление распределения Cp по поверхности
computeCpDistribution(points, triangles, edges, U,
                      INLET_RHO, INLET_U, INLET_P, CHORD,
                      resultsDir, baseName);
